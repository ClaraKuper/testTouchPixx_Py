...
try
    ...
    for i = 1:4
        ...
        Screen('Flip', w);
        moleTimetag = Datapixx('GetLastOnsetTimestamp');
        ...
        while ~whacked
            ...
            status = Datapixx('GetTouchpixxStatus');
            if isfield(status, 'newLogFrames') && status.newLogFrqweames > 0  % We have new TOUCHPixx logged data to read?
                ...
                for iTouch = 1:status.newLogFrames  % Examine each logged TOUCHPixx datum
                    ...
                    if (touchX ~= 0 && touchY ~= 0)  % Confirm datum is not a panel release
                        ...
                        if (abs(whackX - moleX) < moleSize/2 && abs(whackY - moleY) < moleSize/2)
                            ...
                            fprintf('Mole at (%d,%d) whacked in %d milliseconds\n', moleX, moleY, floor(responseTime*1000));
                            break;
                        end
                    end
                end
